...
※改修進行状況
  ✓ログをCSSで自由に整形できます。    → 済
  ✓キャラ名が無い時に、文頭に[font][ruby]等があると改行しない問題に対応しています。    → 公式対応済みっぽい
  ✓[r]で改行しない問題に対応しています。    → マクロにした
  ✓ルビをログに表示します。（CSSで非表示にはできます）    → 切り離し
  ✓同じキャラのセリフが続く場合、キャラ名が重複しないようにできます。    → 済
  ✓デフォルトフォントスタイルをログに反映するかしないか選択可能です。    → 削除予定 → 削除
  ✓[font]タグをログに反映するかしないか選択可能です。    → 切り離し
  ✓[glink]の選択をログに表示できます。※v2.01以降は gling タグで直指定も可能    → 削除（マクロで出来るしいらないかな） → 済（やっぱり作った）
  ✓ログの縦書き対応。※v3.00以降は要パラメータ指定    → 済
  ✓縦書き時の縦中横に対応。※v2.00以降    → 切り離し
  ✓ログのhtmlタグに、[chara_new] で定義した name値が class名として入ります。定義なしの場合は "name"。※2.01以降    → 済（外枠のみ）
  ✓[chara_new]で指定した color をログに反映するか選択可能です。※v3.00以降    → 削除 → 済（やっぱり入れた）
  ✓[font]タグに name パラメータを追加。自由にclass名付けれます。※v3.00以降    → 切り離し
  ✓[mc_ruby][endruby]タグを追加、複数テキストにまとめてルビを振る事が出来ます。（エフェクト非対応）※v3.0    → 切り離し
  ✓[nowait]時、瞬間表示にできます。※v3.10    → 削除（[l][p]で全表示したいけどむずい）
  
  ■追加
  ✓[l] でログがaddになってしまうのでjoinにできる。
  ✓ルビを辞書登録して自動で振れるやつのβ版。クセが凄い。
　
-------------------------------------------------------------------------------------------------
■ 使い方
-------------------------------------------------------------------------------------------------
